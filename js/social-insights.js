EC.Events.subscribe('/ready', function(d) {
	...
	if ($('.zcinput-container').length < 1) {
		var input = new EC.QueryInput('zcinput[]', 'form-control si-input');
  ...
  EC.Prefs.restore('si', ['#twitter-count', '#bias-split', '#enable-cache']);
	...
	$("#btn-classify").on("click", function(e) {
		...
		var query = $("input[name='zcinput[]']").map(function(d) { return $(this).val(); }).get();
		var keyword = $("#keyword").val();
		var tweet_count = $("#twitter-count").val();
    var enable_cache = $("#enable-cache").val();

    EC.Prefs.save('si', ['#twitter-count', '#bias-split', '#enable-cache']);
		...
		$.ajax({
				...
				url: "../api/weighted_search/insights"
			})
			.done(function(d) {
				d3.selectAll(".dynamic-add").remove();
				EC.loading.hide();
				var supLayout = EC.Templates.get("ui-si-layout");
				...
				d.data.combined_insights.queries = d.data.queries;
				buildBubbles(d.data.combined_insights, insertAfter);
			});
	});
	...
	var buildList = function(data) {
		...
		if (users.length > 1) {
			var siList = EC.Templates.get("ui-si-list-multi");
			$(".listBox").append(siList({"categories": unsorted, "users": users}));
		}
		else {
			var siList = EC.Templates.get("ui-si-list-single");
			$(".listBox").append(siList(unsorted));
		}
		// Click events
	...
	var buildBubbles = function(profile, insertAfter) {
		...
		var txtPadding = "20px";
		var data = profile;
		var meta = profile.queries;
		var selector = insertAfter;
		var queries = profile.queries;
		var queryList = profile.query_list;
		...
		var addUserBox = function(queries, queryList, stopEventsForInputBoxes) {
			// do we have multiple users?
			var multipleUsers = (queries.length > 1) ? true : false;
			// user info boxes and query inputs
			var usersAnchor = attachAnchor(selector, txtPadding);
			$(usersAnchor).attr("id", "user-status");
			queries.forEach(function(d, i) {
				var query = queryList.filter(function(n) {
					return n.query == d;
				})[0];
				query.index = i;
				query.selected = false;
				// add the user box
				var boxClass = (multipleUsers) ? "twitter-user-multiple" : "twitter-query-single";
				var box = new EC.QueryBox(query, "col-md-" + Math.floor(12 / queries.length) + " no-padding " + boxClass).element();
				if (multipleUsers) {
